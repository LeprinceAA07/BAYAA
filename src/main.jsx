...
function App() {
  ...
  const [account, setAccount] = useState(() => read('bayaa-account', null));
  ...
  const myOrders = useMemo(() => account ? orders.filter(o => o.customer.email === account.email) : [], [orders, account]);
  const sellerOrders = useMemo(() => account?.role === 'seller' ? orders : [], [orders, account]);
  ...
  useEffect(() => localStorage.setItem('bayaa-account', JSON.stringify(account)), [account]);
  ...
  const createProduct = e => {
    ...
    if (!addForm.title.trim() || price < 1) return;
    const product = { id: `seller-${Date.now()}`, title: addForm.title.trim(), price, category: addForm.category, description: addForm.description.trim() || 'منتج جديد على بياع.', icon: iconByCategory[addForm.category] || Package, sellerCreated: true, sellerEmail: account.email };
  ...
  const placeOrder = e => {
    ...
    const order = { id: `BY-${Date.now().toString().slice(-8)}`, customer: { ...customer, email: account?.email || 'guest@bayaa.local' }, items: cart, total, status: 'جديد', createdAt: new Date().toISOString() };
    setOrders(o => [order, ...o]);
    setCart([]); setCheckoutOpen(false); setOpenCart(false); setOrderDone(order);
  };

  const submitAuth = e => {
    e.preventDefault();
    if (!authForm.email.trim() || !authForm.password.trim() || (accountTab === 'register' && !authForm.name.trim())) return;
    const next = { name: authForm.name.trim() || authForm.email.split('@')[0], email: authForm.email.trim().toLowerCase(), role: accountTab === 'register' ? authForm.role : 'buyer' };
    setAccount(next); setAuthOpen(false); setAuthForm({ name: '', email: '', password: '', role: 'buyer' });
    if (next.role === 'seller') setSellerOpen(true);
  };

  const logout = () => { setAccount(null); setSellerOpen(false); };
  ...
  return <div className="app" dir="rtl">
    <header className="header"><div className="container nav">
      ...
      <div className="actions"><button className="icon" aria-label="الحساب" onClick={() => account ? setSellerOpen(true) : setAuthOpen(true)}><UserRound/></button><button className="cart-btn" onClick={() => setOpenCart(true)} aria-label="السلة"><ShoppingCart/><b>{cart.length}</b></button></div>
    </div></header>
    ...
    <main>
      ...
      <section id="sell" className="sell container"><div><span className="eyebrow">للبائعين</span><h2>لديك منتج؟ ابدأ البيع مع بياع</h2><p>أنشئ متجرك، أضف منتجاتك، ووصل إلى عملاء جدد في موريتانيا.</p></div><button onClick={() => account?.role === 'seller' ? setSellerOpen(true) : setAuthOpen(true)}>ابدأ البيع الآن</button></section>
    </main>
    ...
    {authOpen && <div className="overlay" onClick={() => setAuthOpen(false)}><aside className="modal auth-modal" onClick={e => e.stopPropagation()}><button className="modal-close" onClick={() => setAuthOpen(false)}><X/></button><div className="modal-icon"><Lock/></div><h2>{accountTab === 'login' ? 'تسجيل الدخول' : 'إنشاء حساب'}</h2><p>{accountTab === 'login' ? 'ادخل إلى حسابك في بياع.' : 'أنشئ حساب مشتري أو بائع للبدء.'}</p><div className="auth-tabs"><button className={accountTab === 'login' ? 'active' : ''} onClick={() => setAccountTab('login')}>دخول</button><button className={accountTab === 'register' ? 'active' : ''} onClick={() => setAccountTab('register')}>حساب جديد</button></div><form className="form" onSubmit={submitAuth}>{accountTab === 'register' && <label><User size={17}/> الاسم<input required value={authForm.name} onChange={e => setAuthForm({ ...authForm, name: e.target.value })} placeholder="اسمك"/></label>}<label><Mail size={17}/> البريد الإلكتروني<input required type="email" value={authForm.email} onChange={e => setAuthForm({ ...authForm, email: e.target.value })} placeholder="example@email.com"/></label><label><Lock size={17}/> كلمة المرور<div className="password-field"><input required type={showPassword ? 'text' : 'password'} value={authForm.password} onChange={e => setAuthForm({ ...authForm, password: e.target.value })} placeholder="كلمة المرور"/><button type="button" onClick={() => setShowPassword(!showPassword)}>{showPassword ? <EyeOff size={17}/> : <Eye size={17}/>}</button></div></label>{accountTab === 'register' && <div className="role-switch"><button type="button" className={authForm.role === 'buyer' ? 'active' : ''} onClick={() => setAuthForm({ ...authForm, role: 'buyer' })}>مشتري</button><button type="button" className={authForm.role === 'seller' ? 'active' : ''} onClick={() => setAuthForm({ ...authForm, role: 'seller' })}>بائع</button></div>}<button className="checkout" type="submit">{accountTab === 'login' ? 'دخول' : 'إنشاء الحساب'}</button></form></aside></div>}

    {sellerOpen && <div className="overlay" onClick={() => setSellerOpen(false)}><aside className="seller-panel" onClick={e => e.stopPropagation()}><div className="seller-head"><div><span className="eyebrow">BAYAA ACCOUNT</span><h2>{account?.role === 'seller' ? 'لوحة البائع' : 'حسابي'}</h2></div><button onClick={() => setSellerOpen(false)} aria-label="إغلاق"><X/></button></div>{account?.role === 'seller' ? <><div className="seller-nav"><button className={sellerTab === 'dashboard' ? 'seller-tab active' : 'seller-tab'} onClick={() => setSellerTab('dashboard')}><LayoutDashboard size={17}/> الرئيسية</button><button className={sellerTab === 'products' ? 'seller-tab active' : 'seller-tab'} onClick={() => setSellerTab('products')}><Package size={17}/> المنتجات</button><button className={sellerTab === 'orders' ? 'seller-tab active' : 'seller-tab'} onClick={() => setSellerTab('orders')}><ClipboardList size={17}/> الطلبات</button></div>{sellerTab === 'dashboard' && <div className="seller-content"><div className="seller-stats"><div><strong>{sellerOrders.length}</strong><span>الطلبات</span></div><div><strong>{sellerSales.toLocaleString('ar-MR')} MRU</strong><span>المبيعات</span></div><div><strong>{sellerProducts.filter(p => p.sellerEmail === account.email).length}</strong><span>المنتجات</span></div></div><div className="seller-card"><Store/><div><h3>مرحبًا {account.name}</h3><p>حساب البائع جاهز لإدارة منتجاتك وطلباتك.</p></div><button onClick={() => setSellerTab('products')}>إضافة منتج</button></div></div>}{sellerTab === 'products' && <div className="seller-content"><form className="form seller-form" onSubmit={createProduct}><label>اسم المنتج<input required value={addForm.title} onChange={e => setAddForm({ ...addForm, title: e.target.value })} placeholder="مثال: هاتف Samsung"/></label><label>السعر بـ MRU<input required type="number" min="1" value={addForm.price} onChange={e => setAddForm({ ...addForm, price: e.target.value })} placeholder="5000"/></label><label>القسم<select value={addForm.category} onChange={e => setAddForm({ ...addForm, category: e.target.value })}>{categories.slice(1).map(([name]) => <option key={name}>{name}</option>)}</select></label><label>الوصف<textarea value={addForm.description} onChange={e => setAddForm({ ...addForm, description: e.target.value })} placeholder="وصف المنتج"/></label><button className="checkout" type="submit"><Plus size={18}/> نشر المنتج</button></form><div className="seller-list">{sellerProducts.filter(p => p.sellerEmail === account.email).map(p => <div className="seller-row" key={p.id}><Package/><div><strong>{p.title}</strong><span>{Number(p.price).toLocaleString('ar-MR')} MRU</span></div><button onClick={() => setSellerProducts(list => list.filter(x => x.id !== p.id))}><Trash2 size={17}/></button></div>)}</div></div>}{sellerTab === 'orders' && <div className="seller-content"><div className="order-list">{sellerOrders.length ? sellerOrders.map(o => <div className="order-card" key={o.id}><div><strong>{o.id}</strong><span>{o.customer.name} — {o.customer.phone}</span></div><b>{Number(o.total).toLocaleString('ar-MR')} MRU</b><small>{o.status}</small></div>) : <div className="empty"><ClipboardList/><p>لا توجد طلبات بعد.</p></div>}</div></div>}<button className="seller-logout" onClick={logout}><LogOut size={17}/> تسجيل الخروج</button></> : <div className="seller-content"><div className="seller-card"><UserRound/><div><h3>{account?.name}</h3><p>{account?.email}</p></div></div><div className="seller-card"><ClipboardList/><div><h3>طلباتي</h3><p>لديك {myOrders.length} طلبات محفوظة على هذا الجهاز.</p></div><button onClick={() => setSellerTab('orders')}>عرض</button></div><div className="order-list">{myOrders.map(o => <div className="order-card" key={o.id}><div><strong>{o.id}</strong><span>{o.customer.city} — {o.customer.address}</span></div><b>{Number(o.total).toLocaleString('ar-MR')} MRU</b><small>{o.status}</small></div>)}</div><button className="seller-logout" onClick={logout}><LogOut size={17}/> تسجيل الخروج</button></div>}</aside></div>}
  </div>;
}
