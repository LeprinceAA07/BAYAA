...
const tiers = [
  {
    ...
    priceId: {
      month: import.meta.env.BAYAA_PADDLE_STARTER_MONTH_PRICE_ID || '',
      year: import.meta.env.BAYAA_PADDLE_STARTER_YEAR_PRICE_ID || '',
    },
  },
  {
    name: 'Pro',
    ...
    priceId: {
      month: import.meta.env.BAYAA_PADDLE_PRO_MONTH_PRICE_ID || '',
      year: import.meta.env.BAYAA_PADDLE_PRO_YEAR_PRICE_ID || '',
    },
  },
  {
    name: 'Advanced',
    ...
    priceId: {
      month: import.meta.env.BAYAA_PADDLE_ADVANCED_MONTH_PRICE_ID || '',
      year: import.meta.env.BAYAA_PADDLE_ADVANCED_YEAR_PRICE_ID || '',
    },
  },
];
...
function PricingPage({ countryCode, signedInEmail }) {
  ...
  const selected = useMemo(
    () => tiers.map((tier) => ({ ...tier, selectedPriceId: tier.priceId[billing] })),
    [billing],
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const environment = String(import.meta.env.BAYAA_PADDLE_ENVIRONMENT || '')
          .trim()
          .toLowerCase();
        ...
        if (environment !== 'sandbox') {
          throw new Error('This pricing page is configured for Paddle Sandbox only.');
        }
        if (!token.startsWith('test_')) {
          throw new Error('A Paddle Sandbox client-side token starting with test_ is required.');
        }
  ...
  useEffect(() => {
    ...
    const missing = selected
      .filter((tier) => !tier.selectedPriceId)
      .map((tier) => `${tier.name} ${billing}`);
      ...
      <div className="pricing-container">
        ...
        <section className="pricing-grid">
          {selected.map((tier, index) => {
            ...
            const displayTotal = isStarter ? '$0.00' : price?.total;
        ...
        <p className="sandbox-note">
          Paddle Sandbox • الأسعار المعروضة هي totals المرسلة من Paddle مباشرة، بدون حساب أو إعادة تنسيق على الواجهة.
        </p>
      </div>
    </main>
  );
